# -*- coding: utf-8 -*-
"""Звуковая волна — единый индикатор экранов:
idle (главный/очередь, анимация) · progress (обработка).

Внимание: имя модуля совпадает со stdlib `wave` (его использует asr.py) —
внутри пакета импортировать только как `from .wave import ...`/`censr.wave`."""
